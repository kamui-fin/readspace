...
function xmlGatherAttributesToOutline(
  element: Element,
  outline: OpmlOutline,
): void {
  if (element.attributes) {
    for (let i = 0; i < element.attributes.length; i++) {
      ...
      if (att?.specified && att.name && att.value !== undefined) {
        outline[att.name] = att.value;
      }
    }
  }
}
...
export interface OpmlOutline {
  ...
  type?: string;
  title?: string;
  // All additional attributes in OPML are string values
  [key: string]: string | OpmlOutline[] | undefined;
  subs?: OpmlOutline[];
}
...
function outlineToJson(
  element: Element,
  nameOutlineElement = "outline",
): OpmlOutline {
  const theOutline: OpmlOutline = {};

  xmlGatherAttributesToOutline(element, theOutline);

  if (xmlHasSubs(element)) {
    theOutline.subs = [];
    const outlineChildren = element.querySelectorAll(
      `:scope > ${nameOutlineElement}`,
    );
    outlineChildren.forEach((child) => {
      theOutline.subs!.push(
        outlineToJson(child as Element, nameOutlineElement),
      );
    });
  }
    ...
    if (subs !== undefined) {
      for (let i = 0; i < subs.length; i++) {
        ...
        for (const x in sub) {
          if (x !== "subs") {
            const value = sub[x];
            // Only include string attributes in the output
            if (typeof value === "string") {
              atts += ` ${x}="${encodeXml(value)}"`;
            }
          }
        }
        if (sub && sub.subs === undefined) {
          add(`<outline${atts} />`);
        } else if (sub && sub.subs) {
          add(`<outline${atts} >`);
          indentlevel++;
          addSubs(sub.subs);
          add("</outline>");
...
function visitAll(
  theOutline: OpmlStructure,
  callback: (outline: OpmlOutline) => boolean,
): void {
  function visitSubs(theNode: { subs?: OpmlOutline[] }): boolean {
    if (theNode.subs !== undefined) {
      for (let i = 0; i < theNode.subs.length; i++) {
        ...
        if (theSub && !callback(theSub)) {
          return false;
        }
        if (theSub && !visitSubs(theSub)) {
          return false;
        }
      }
    }
