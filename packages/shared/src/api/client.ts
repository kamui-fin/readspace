...
export class ApiClient {
    ...
    static books = {
        ...
        createBook: (data: BookMetadataCreate) => this.post<UserBookLibrary>("/api/books/", data),
        updateBook: (id: string, data: UserBookLibraryUpdate) => this.put<UserBookLibrary>(`/api/books/${id}`, data),
    ...
    static highlights = {
        ...
        createHighlight: (data: HighlightCreate) => this.post<Highlight>("/api/highlights/", data),
    ...
    static rss = {
        ...
        importOPML: (formData: FormData) =>
            this.uploadFile("/api/rss/opml/import/", formData) as Promise<OPMLImportResponse>,
        getImportTaskStatus: (taskId: string) =>
            this.get<ImportTaskStatus>(`/api/rss/opml/import/status/${taskId}`),
        getActiveImportTask: () => this.get<ImportTaskStatus | null>("/api/rss/opml/import/active"),
        listImportTasks: () => this.get<ActiveImportTask[]>("/api/rss/opml/import/tasks"),
        ...
        getFeeds: (params?: {
            ...
            return this.get<Feed[]>(`/api/rss/feeds/${queryString ? `?${queryString}` : ""}`)
        },
        ...
        getSimilarFeeds: (
            ...
            if (params?.min_similarity)
                queryParams.append(
                    "min_similarity",
                    params.min_similarity.toString()
                )

            const queryString = queryParams.toString()
            return this.get<SimilarFeedsResponse>(`/api/rss/similar/${id}${queryString ? `?${queryString}` : ""}`)
        },
        ...
        getArticles: (params: {
            ...
            const queryString = queryParams.toString()
            return this.get<ArticlesPaginatedResponse>(`/api/rss/articles/${queryString ? `?${queryString}` : ""}`)
        },
        getRecentlyReadArticles: (page?: number, size?: number): Promise<ArticlesPaginatedResponse> => {
            const queryParams = new URLSearchParams()
        ...
        getReadLaterArticles: (page?: number, size?: number): Promise<ArticlesPaginatedResponse> => {
            const queryParams = new URLSearchParams()
            if (page) queryParams.append("page", page.toString())
            if (size) queryParams.append("size", size.toString())
        ...
        getTodaysArticles: (params?: { page?: number; size?: number }): Promise<ArticlesPaginatedResponse> => {
            const queryParams = new URLSearchParams()
            if (params?.page) queryParams.append("page", params.page.toString())
            if (params?.size) queryParams.append("size", params.size.toString())

            const queryString = queryParams.toString()
            return this.get<ArticlesPaginatedResponse>(`/api/rss/articles/today${queryString ? `?${queryString}` : ""}`)
        },
