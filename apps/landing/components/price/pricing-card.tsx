      ...
      `}
        >
            <div className="mb-6">
                ...
                <h3
                    className={`text-xl font-bold mt-4 ${includesFree ? "text-[#1A1A1A]" : "text-[#1A1A1A]/80"}`}
                >
                    {title}
                </h3>
                <p
                    className={`text-sm mt-1 ${includesFree ? "text-[#1A1A1A]/60" : "text-[#1A1A1A]/50"}`}
                >
                    {description}
                </p>
            </div>
            ...
            <div className="mb-6">
                <div className="flex items-baseline">
                    <span
                        className={`text-4xl font-bold ${includesFree ? "text-[#1A1A1A]" : "text-[#1A1A1A]/80"}`}
                    >
                        {price}
                    </span>
                    <span
                        className={`ml-1 ${includesFree ? "text-[#1A1A1A]/60" : "text-[#1A1A1A]/50"}`}
                    >
                        {pricePeriod}
                    </span>
                </div>
            ...
            <ul className="space-y-3 mb-8">
                {features.map((feature, i) => (
                    <li key={i} className="flex">
                        ...
                        />
                        <span
                            className={`text-sm ${includesFree ? "text-[#1A1A1A]/80" : "text-[#1A1A1A]/70"}`}
                        >
                            {feature}
                        </span>
                    </li>
