...
export function scanForFeeds(): DetectedFeed[] {
    ...
    const addFeed = (feed: DetectedFeed) => {
        try {
            ...
            if (absoluteUrl.match(/\.(jpg|jpeg|png|gif|svg|css|js|woff|woff2|ttf|eot|mp4|webm|pdf|zip|tar|gz)$/i)) {
                return
            }

            // Skip common asset directories
            if (absoluteUrl.match(/\/(static|assets|images?|img|media|js|css|fonts?|dist|build|node_modules)\//i)) {
                return
            }
