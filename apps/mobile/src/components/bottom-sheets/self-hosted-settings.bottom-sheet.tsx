...
export const SelfHostSettingsBottomSheet = forwardRef<
	BottomSheetModal,
	SelfHostSettingsProps
>(({ onSave, onClose, initialData }, ref) => {
	const [apiUrl, setApiUrl] = useState(initialData?.apiUrl || "");
	const [supabaseUrl, setSupabaseUrl] = useState(
		initialData?.supabaseUrl || "",
	);
	const [supabaseAnonKey, setSupabaseAnonKey] = useState(
		initialData?.supabaseAnonKey || "",
	);
	...
	useEffect(() => {
		...
		return () =>
			console.log("[SelfHostSettingsBottomSheet] Component unmounted");
	}, []);
	...
	const handleSave = useCallback(async () => {
		...
		try {
			...
			const validationPromise = (async () => {
				...
				console.log(
					"[SelfHostSettingsBottomSheet] Supabase URL:",
					data.supabaseUrl,
				);
				...
				console.log(
					"[SelfHostSettingsBottomSheet] Resolved API URL:",
					resolvedApiUrl,
				);
				...
				try {
					...
					if (!apiResponse.ok) {
						console.log(
							"[SelfHostSettingsBottomSheet] API endpoint failed:",
							apiResponse.status,
						);
						throw new Error(
				...
				console.log(
					"[SelfHostSettingsBottomSheet] Testing Supabase connection...",
				);
