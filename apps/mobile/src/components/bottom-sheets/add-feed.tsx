    ...
    ({ onConfirm }, ref) => {
        ...
        return (
            <BottomSheet
                ...
                {/* Subtitle */}
                <Text className="font-geist-regular text-base text-grey dark:text-grey mb-4">
                ...
                {isPreviewing && (
                    <View className="mt-8 items-center justify-center">
                        ...
                        <Text className="font-geist mt-3 text-sm text-grey dark:text-grey">Finding feed...</Text>
                    </View>
                )}
                ...
                {!isPreviewing && feedPreview && (
                    <View
                        className="mt-4 p-4 rounded-xl border border-divider flex-col gap-3"
                        ...
                        <View className="flex-row items-start gap-3">
                            {/* Avatar */}
                            <View
                                className="h-14 w-14 items-center justify-center overflow-hidden rounded-xl mt-0.5"
                            ...
                            <View className="flex-1">
                                <Text size="base" fontFamily="geist-semibold" className="text-black mb-1.5 tracking-tight" numberOfLines={2}>
                                ...
                                <View className="flex-row flex-wrap items-center gap-x-3 gap-y-1.5">
                                    ...
                                    {feedPreview.author ? (
                                        <View className="flex-row items-center gap-1">
                                            <UserCircleLinearIcon width={12} height={12} color={colors.grey} strokeWidth={1.8} />
                                            <Text size="sm" fontFamily="geist" style={{ color: colors.grey, fontSize: 11 }} numberOfLines={1}>
                                                {feedPreview.author}
                                            </Text>
                                        </View>
                                    ) : null}
                                    ...
                                    {feedPreview.content_type ? (
                                        <View className="flex-row items-center gap-1">
                                            <LayersMinimalisticLinearIcon width={12} height={12} color={colors.grey} strokeWidth={1.8} />
                                    ...
                                    {feedPreview.language ? (
                                        <View className="rounded px-1.5 py-0.5" style={{ backgroundColor: colors.grey5 }}>
                                            <Text fontFamily="geist-medium" style={{ color: colors.grey, fontSize: 8, letterSpacing: 0.3, textTransform: 'uppercase' }}>
                                                {feedPreview.language}
                                            </Text>
                                        </View>
                                    ) : null}
                        ...
                        {(feedPreview.link || feedPreview.url) ? (
                            <Pressable
                                ...
                                <LinkMinimalistic2BoldIcon width={12} height={12} strokeWidth={2.4} color={colors.primary} />
                                <Text size="sm" fontFamily="geist" className="flex-1 flex-shrink text-left" style={{ color: colors.primary, fontSize: 11 }} numberOfLines={1}>
                                    {feedPreview.link || feedPreview.url}
                                </Text>
                            </Pressable>
                        ) : null}
                        ...
                        {/* Tags Badges */}
                        {(() => {
                            const displayTags = feedPreview.tags_native && feedPreview.tags_native.length > 0
                                    ...
                                    {displayTags.slice(0, 4).map((tag: string, index: number) => {
                                        ...
                                        return (
                                            <View
                                                ...
                                                style={{ backgroundColor: colors.grey5 }}>
                                                <Text size="sm" fontFamily="geist" style={{ color: colors.grey, fontSize: 10 }}>
                                                    #{formattedTag}
                                                </Text>
                                            </View>
                                        );
