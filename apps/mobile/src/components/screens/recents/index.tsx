...
export function RecentsScreen() {
	...
	const listItems = useMemo(() => {
		...
		const articlesWithDates: ArticleWithDate[] = allArticles.map(
			(article: Article) => ({
				...article,
	...
	const renderItem = (item: ListItem) => {
		...
		if (item.type === "divider") {
			return (
				<View className="mx-4 h-[0.5px] bg-light-grey dark:bg-mid-grey-dark" />
			);
		}
		...
		if (item.type === "article" && item.data) {
			...
			if (isClipped && article.link) {
				...
				typeof article.feed === "object" &&
				article.feed &&
				(article.feed as any).link
			) {
