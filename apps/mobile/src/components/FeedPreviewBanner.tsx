...
export function FeedPreviewBanner({ feedTitle, onFollow }: FeedPreviewBannerProps) {
    return (
        <View className="w-full border-b border-light-grey bg-orange-50 px-4 py-3">
            <View className="flex-row items-center justify-between gap-3">
                <View className="flex-1 flex-row items-center gap-4">
                    <Monicon name="solar:eye-linear" size={18} color="#EA580C" />
                    <View className="flex-1">
                        <Text className="font-geist-semibold text-sm text-orange-700">
                            Preview Mode
                        </Text>
                        {feedTitle && (
                            <Text className="font-geist text-xs text-orange-600" numberOfLines={1}>
                ...
                {onFollow && (
                    <Button
                        onPress={onFollow}
                        variant="secondary"
                        className="h-8 flex-row items-center gap-1.5 bg-orange-600 px-3">
