...
export const SettingsGroup = ({ title, children, className }: SettingsGroupProps) => {
    return (
        <View className={cn('gap-2', className)}>
            <Text className="font-geist-semibold text-sm text-grey">{title}</Text>
            <View className="overflow-hidden rounded-2xl bg-light-grey">{children}</View>
        </View>
    );
};
    ...
    ({ label, variant, value, icon, className, isLast = false, ...props }, ref) => {
                ...
                case 'select':
                    return (
                        <View className="flex-row items-center gap-2">
                            <Text className="font-geist text-base text-grey">{value}</Text>
        ...
        return (
            <Pressable
                ref={ref}
                className={cn(
                    'flex-row items-center justify-between bg-white px-4 py-4 pl-0 transition-opacity active:opacity-80',
                    !isLast && 'border-b border-light-grey',
                    className
                )}
                {...props}>
                <Text className="font-geist text-base text-black">{label}</Text>
                {renderRightContent()}
            </Pressable>
        );
    }
