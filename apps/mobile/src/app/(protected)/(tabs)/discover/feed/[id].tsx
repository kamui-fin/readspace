...
export default function FeedPreviewScreen() {
	...
	const [pendingSimilarFeedUrl, setPendingSimilarFeedUrl] = useState<
		string | null
	>(null);
	const [isPreviewRefreshing, setIsPreviewRefreshing] = useState(false);
	const [previewFeedData, setPreviewFeedData] =
		useState<FeedDiscoveryResult | null>(null);
	const colorScheme = useColorScheme();

	// Local state to track subscription status for immediate UI updates
	const [localIsSubscribed, setLocalIsSubscribed] = useState<boolean | null>(
		null,
	);
	...
	const { data: fetchedFeedData, isLoading: isFeedLoading } = useFeed(
		id || "",
		{
	...
	const { data: similarData, isLoading: isSimilarLoading } =
	...
	const handleArticlePress = useCallback(
		(articleId: string) => {
			...
			router.push(
				`/(protected)/articles/${articleId}?isSubscribed=${isSubscribed}`,
			);
		},
