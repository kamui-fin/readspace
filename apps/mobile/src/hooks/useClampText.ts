...
export const useClampText = ({
  ...
  const [innerText, setInnerText] = useState<string | Iterable<ReactNode> | null>(() =>
  ...
  useEffect(() => {
    if (isFirstLoad.current) {
      ...
      const newText = typeof text === 'string' ? text.replace(/[\r\n]/g, ' ') : text;
  ...
  const onTextLayout = (e: NativeSyntheticEvent<TextLayoutEventData>) => {
    ...
    const text = `${finalText.slice(0, finalText.length - expandButtonWidth)}${ellipsis}`;
