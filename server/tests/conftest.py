...
async def create_test_user(session: AsyncSession, user_id: str = None, email: str = None) -> Profile:
    ...
    await session.execute(
        text("INSERT INTO auth.users (id, aud, role, email, encrypted_password, email_confirmed_at, invited_at, confirmation_token, confirmation_sent_at, recovery_token, recovery_sent_at, email_change_token_new, email_change, email_change_sent_at, last_sign_in_at, raw_app_meta_data, raw_user_meta_data, is_super_admin, created_at, updated_at, phone, phone_confirmed_at, phone_change, phone_change_token, phone_change_sent_at, email_change_token_current, email_change_confirm_status, banned_until, reauthentication_token, reauthentication_sent_at, is_sso_user, deleted_at, is_anonymous) VALUES (:user_id, 'authenticated', 'authenticated', :email, '', NOW(), NULL, '', NOW(), '', NOW(), '', '', NOW(), NOW(), '{}', '{}', FALSE, NOW(), NOW(), NULL, NULL, '', '', NOW(), '', 0, NULL, '', NOW(), FALSE, NULL, FALSE) ON CONFLICT (id) DO NOTHING"),
...
async def create_test_book(session: AsyncSession, **kwargs):
    ...
    import uuid
    from datetime import datetime, timezone
    from app.models.book_models import BookMetadata, BookFormat
