...
from app.models import Feed, Profile, Subscription
...
class TestGetSimilarFeeds:
    ...
    async def test_get_similar_feeds_success(
        ...
        response = client.get(f"/api/v1/similar/{test_feed.id}")
    ...
    async def test_get_similar_feeds_with_limit(
        ...
        response = client.get(f"/api/v1/similar/{test_feed.id}?limit=5")
    ...
    async def test_get_similar_feeds_with_min_similarity(
        ...
        response = client.get(f"/api/v1/similar/{test_feed.id}?min_similarity=0.5")
    ...
    def test_get_similar_feeds_not_subscribed(self, client: TestClient, test_feed: Feed):
        """Test getting similar feeds for non-subscribed feed."""
        response = client.get(f"/api/v1/similar/{test_feed.id}")

        assert response.status_code == 404

    def test_get_similar_feeds_not_found(self, client: TestClient):
        """Test getting similar feeds for non-existent feed."""
        from uuid import uuid4

        fake_id = uuid4()
        response = client.get(f"/api/v1/similar/{fake_id}")

        assert response.status_code == 404

    def test_get_similar_feeds_invalid_uuid(self, client: TestClient):
        """Test with invalid UUID format."""
        response = client.get("/api/v1/similar/invalid-uuid")

        assert response.status_code == 422

    def test_get_similar_feeds_invalid_limit(self, client: TestClient, test_feed: Feed):
        """Test with invalid limit parameter."""
        response = client.get(f"/api/v1/similar/{test_feed.id}?limit=0")

        assert response.status_code == 422

    def test_get_similar_feeds_invalid_similarity(self, client: TestClient, test_feed: Feed):
        """Test with invalid similarity threshold."""
        response = client.get(f"/api/v1/similar/{test_feed.id}?min_similarity=2.0")
