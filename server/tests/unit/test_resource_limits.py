...
async def test_enforce_subscription_limit_under_limit():
    ...
    with patch("app.services.user.resource_limits.get_profile_by_id", return_value=profile), patch(
        "app.services.user.resource_limits.get_current_usage", return_value=2
    ):
...
async def test_enforce_subscription_limit_exceeded():
    ...
    with patch("app.services.user.resource_limits.get_profile_by_id", return_value=profile), patch(
        "app.services.user.resource_limits.get_current_usage", return_value=5
    ):
...
async def test_enforce_daily_ai_limit_under_limit():
    ...
    with patch("app.services.user.resource_limits.get_profile_by_id", return_value=profile), patch(
        "app.services.user.resource_limits.redis_cache.incr", return_value=1
    ) as mock_incr:
...
async def test_enforce_daily_ai_limit_exceeded():
    ...
    with patch("app.services.user.resource_limits.get_profile_by_id", return_value=profile), patch(
        "app.services.user.resource_limits.redis_cache.incr", return_value=4
    ) as mock_incr, patch("app.services.user.resource_limits.redis_cache.decr") as mock_decr:
