...
class TestLazyArticleStateCreation:
    ...
    def test_create_articles_batch_signature_no_user_id(self):
        ...
        sig = inspect.signature(ArticleCrudOperations.create_articles_batch)
    ...
    def test_docstring_documents_lazy_creation(self):
        ...
        docstring = ArticleCrudOperations.create_articles_batch.__doc__
    ...
    def test_create_articles_batch_source_does_not_reference_user_article_states(self):
        ...
        source = inspect.getsource(ArticleCrudOperations.create_articles_batch)
...
class TestStateCreationOnUserInteraction:
    ...
    def test_get_or_create_user_article_state_function_exists(self):
        ...
        from app.crud.article.user_article_state import get_or_create_user_article_state

        assert callable(get_or_create_user_article_state)
    ...
    def test_update_article_read_status_function_exists(self):
        ...
        from app.crud.article.user_article_state import update_article_read_status

        assert callable(update_article_read_status)

    def test_toggle_article_favorite_uses_get_or_create(self):
        """Verify toggle_article_favorite uses get_or_create pattern."""
        import inspect

        from app.crud.article.user_article_state import toggle_article_favorite

        source = inspect.getsource(toggle_article_favorite)

        # Should call get_or_create_user_article_state to ensure state exists
        assert "get_or_create_user_article_state" in source

    def test_toggle_article_read_later_uses_get_or_create(self):
        """Verify toggle_article_read_later uses get_or_create pattern."""
        import inspect

        from app.crud.article.user_article_state import toggle_article_read_later

        source = inspect.getsource(toggle_article_read_later)
    ...
    def test_update_article_status_uses_upsert_pattern(self):
        ...
        source = inspect.getsource(ArticleCrudOperations.update_article_status)
...
class TestUnreadArticleLogic:
    ...
    def test_get_user_unread_count_function_exists(self):
        ...
        from app.crud.article.user_article_state import get_user_unread_count

        assert callable(get_user_unread_count)
    ...
    def test_unread_count_uses_outer_join_pattern(self):
        ...
        source = inspect.getsource(get_user_unread_count)
    ...
    def test_article_query_builder_supports_null_states(self):
        ...
        query_builder = ArticleQueryBuilder(user_id=uuid4())

        # Build a basic query - should not raise errors
        try:
            stmt = query_builder.build_filtered_query(is_read=False, skip=0, limit=10)
            assert stmt is not None
        except Exception as e:
            pytest.fail(f"ArticleQueryBuilder should handle null states: {e}")
