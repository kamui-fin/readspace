...
from app.schemas.subscription_schemas import UserArticleStateCreate, UserArticleStateUpdate
...
class TestUserArticleStateCRUD:
    ...
    async def test_update_user_article_state_mark_read(self, mock_db, sample_user_article_state):
        ...
        with patch('app.crud.crud_user_article_state.datetime') as mock_datetime:
            ...
            result = await update_user_article_state(
                db=mock_db, 
                state_db=sample_user_article_state, 
                state_in=state_update
            )
    ...
    async def test_get_user_unread_count(self, mock_db):
        ...
        mock_result = Mock()
        mock_result.scalars().all.return_value = [uuid4(), uuid4(), uuid4()]  # 3 articles
        mock_db.execute = AsyncMock(return_value=mock_result)
