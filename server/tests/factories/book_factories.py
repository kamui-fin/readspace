...
from app.models.book_models import BookMetadata, UserBookLibrary, Highlight, BookFormat, HighlightColor
...
class BookMetadataFactory(SQLAlchemyModelFactory):
    ...
    epub_chapter_char_counts = factory.LazyAttribute(
        ...
        if obj.format == BookFormat.EPUB else None
    )
    ...
    pdf_toc = factory.LazyAttribute(
        ...
        if obj.format == BookFormat.PDF else None
    )
...
class UserBookLibraryFactory(SQLAlchemyModelFactory):
    ...
    epub_progress = factory.LazyAttribute(
        lambda obj: {"current_chapter": 0, "current_page": 0, "progress_percentage": 0.0}
        if obj.book_metadata and obj.book_metadata.format == BookFormat.EPUB else None
    )
...
class HighlightFactory(SQLAlchemyModelFactory):
    ...
    color = factory.Faker("random_element", elements=[
        HighlightColor.YELLOW, HighlightColor.GREEN, HighlightColor.BLUE
    ])
    ...
    html_range = factory.LazyFunction(lambda: {
        "startContainer": "p[0]",
        "startOffset": 10,
        "endContainer": "p[0]", 
        "endOffset": 50
    })
    pdf_rect_position = factory.LazyFunction(lambda: {
        "x": 100,
        "y": 200,
        "width": 300,
        "height": 20,
