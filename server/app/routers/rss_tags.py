...
async def create_tag(
    *, 
    ...
    try:
        ...
        logger.info("Tag created successfully", tag_id=tag.id, user_id=current_user.sub, tag_name=tag.name)
        return tag
    except ValueError as e:
        logger.warning("Failed to create tag due to value error", error=str(e), user_id=current_user.sub, tag_name=tag_in.name)
...
async def get_tag(
    ...
    if not tag:
        logger.warning("Tag not found or access denied", tag_id=tag_id, user_id=current_user.sub)
...
async def update_tag(
    ...
    try:
        ...
        if not updated_tag:
            logger.warning("Tag not found for update or access denied", tag_id=tag_id, user_id=current_user.sub)
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Tag not found")
        logger.info("Tag updated successfully", tag_id=updated_tag.id, user_id=current_user.sub)
        return updated_tag
    except ValueError as e:
        logger.warning("Failed to update tag due to value error", error=str(e), user_id=current_user.sub, tag_id=tag_id)
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
...
async def delete_tag(
    ...
    try:
        ...
        if not success:
            logger.warning("Tag not found for deletion or access denied", tag_id=tag_id, user_id=current_user.sub)
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Tag not found")
        logger.info("Tag deleted successfully", tag_id=tag_id, user_id=current_user.sub)
        return # Returns 204 No Content on success
    except ValueError as e: # Should not typically be raised by tag deletion unless logic changes
        logger.warning("Failed to delete tag due to value error", error=str(e), user_id=current_user.sub, tag_id=tag_id)
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
        logger.error("Unexpected error deleting tag", error=str(e), user_id=current_user.sub, tag_id=tag_id)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An unexpected error occurred while deleting the tag.") 