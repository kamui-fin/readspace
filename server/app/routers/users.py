...
async def delete_account(
    ...
    Deletes the Supabase auth user; the `profiles` row and everything that
    cascades from it (feeds, folders, articles, codex data) are removed via
    `ON DELETE CASCADE` foreign keys.
    """
    logger.bind(user_id=current_user.sub)
    await account_service.delete_account(UUID(current_user.sub))
