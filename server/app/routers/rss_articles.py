...
async def list_articles(
    db: AsyncSession = Depends(get_db),
    current_user: TokenData = Depends(get_current_user),
    feed_ids: list[UUID] | None = Query(
        None, description="Filter by specific feed IDs"
    ),
    ...
    is_favorite: bool | None = Query(
        None, description="Filter by article favorite status"
    ),
    feed_is_favorite: bool | None = Query(
        None, description="Filter by parent feed's favorite status"
    ),
    published_since: datetime | None = Query(
        None, description="Filter by articles published since this UTC datetime"
    ),
...
async def get_unread_article_counts(
    ...
    folder_id: UUID | None = Query(
        None, description="Optional folder ID to get counts for a specific folder"
    ),
):
