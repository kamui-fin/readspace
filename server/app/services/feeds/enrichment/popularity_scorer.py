...
class DomainAuthorityScorer:
    ...
    def __init__(self, page_rank_loader: Any | None = None) -> None:
    ...
    def score_domain_authority(self, domain: str, xml_url: str = "") -> int:
        ...
        if self.page_rank_loader:
            ...
            if page_rank_score > 0.0:
                logger.debug("PageRank found for domain", domain=clean_domain, score=page_rank_score)
                return int(page_rank_score)
            else:
                logger.debug("PageRank not found for domain", domain=clean_domain, score=0)
...
class PopularityScorer:
    ...
    def calculate_popularity_score(self, feed_data: dict[str, Any]) -> dict[str, Any]:
        ...
        xml_url = feed_data.get("xmlUrl", "")
        logger.debug(
            f"PopularityScorer: Looking up domain '{domain}' for feed: {feed_data.get('title', 'Unknown')[:50]}"
        )
        if xml_url:
