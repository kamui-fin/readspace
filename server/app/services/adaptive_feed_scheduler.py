...
from app.models.rss_models import ArticleContent, Feed, FeedArticle
...
async def calculate_optimal_interval(db: AsyncSession, feed: Feed) -> int:
    ...
    """
    # If feed has no articles yet, use default
    if not feed.last_article_published_at:
...
def calculate_error_backoff_interval(consecutive_errors: int) -> int:
    ...
    # Exponential backoff: 2^n hours
    # 1 error → 2 hours
    # 2 errors → 4 hours
    # 3 errors → 8 hours
    # 4+ errors → 12 hours (capped)
