...
class RssOrchestrationService:
    ...
    async def get_read_later_articles(self, skip: int = 0, limit: int = 50) -> PaginatedResponse[ArticleResponse]:
        """Get read later articles."""
        return await self.article_service.get_read_later_articles(skip=skip, limit=limit)

    async def get_recently_read_articles(self, skip: int = 0, limit: int = 50) -> PaginatedResponse[ArticleResponse]:
        """Get recently read articles."""
        return await self.article_service.get_recently_read_articles(skip=skip, limit=limit)
    ...
    async def count_unread_articles_by_folder(self, folder_id: UUID) -> int:
        ...
        return await self.article_service.count_unread_articles_by_folder(folder_id=folder_id)

    async def get_total_unread_count(self) -> int:
        """Get total unread count."""
        return await self.article_service.get_total_unread_count()

    async def get_unread_counts(self, folder_id_filter: UUID | None = None) -> dict[str, Any]:
        """Get unread article counts."""
        if folder_id_filter:
            count = await self.article_service.count_unread_articles_by_folder(folder_id=folder_id_filter)
            ...
            unread_by_folder = [
                {"folder_id": str(folder_id), "unread_count": count} for folder_id, count in folder_counts.items()
            ]
            return {
