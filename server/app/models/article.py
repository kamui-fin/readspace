...
class ArticleContent(Base):
    ...
    estimated_read_time_minutes = Column(Integer, nullable=False, default=0)
...
class FeedArticle(Base):
    ...
    content_id = Column(SQLUUID(as_uuid=True), ForeignKey("article_contents.id", ondelete="CASCADE"), nullable=False)
...
class UserEntry(Base):
    ...
    content_id = Column(SQLUUID(as_uuid=True), ForeignKey("article_contents.id", ondelete="CASCADE"), nullable=True)
    feed_article_id = Column(SQLUUID(as_uuid=True), ForeignKey("feed_articles.id", ondelete="CASCADE"), nullable=True)

    # State flags
    is_read = Column(Boolean, nullable=False, server_default="false")
    is_saved = Column(Boolean, nullable=False, server_default="false")
    priority = Column(SQLEnum(ArticlePriority, name="articlepriority"), nullable=False, server_default="LOW")
