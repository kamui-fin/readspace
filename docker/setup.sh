...
# Generate or reuse INBOUND_WEBHOOK_SECRET
if [ "$REUSE_SECRETS" = true ] && grep -q "^INBOUND_WEBHOOK_SECRET=." "$DOCKER_ENV" && ! grep -q "^INBOUND_WEBHOOK_SECRET=dev_inbound_secret" "$DOCKER_ENV"; then
    INBOUND_WEBHOOK_SECRET=$(grep "^INBOUND_WEBHOOK_SECRET=" "$DOCKER_ENV" | cut -d'=' -f2-)
else
    INBOUND_WEBHOOK_SECRET=$(openssl rand -hex 32)
fi

set_env_var "$DOCKER_ENV" "MEILISEARCH_MASTER_KEY" "$MEILISEARCH_MASTER_KEY"
set_env_var "$DOCKER_ENV" "INBOUND_WEBHOOK_SECRET" "$INBOUND_WEBHOOK_SECRET"
...
echo "✅ Meilisearch master key, webhook secret, and deployment URLs saved to docker/.env."

# Validate that initial config was written correctly
if ! validate_env_file "$SCRIPT_DIR/.env" \
    "MEILISEARCH_MASTER_KEY" "INBOUND_WEBHOOK_SECRET" "RSSHUB_MODE"; then
    exit 1
fi
...
if [ "$REUSE_SECRETS" = true ]; then
    ...
    if grep -q "^DASHBOARD_PASSWORD=." "$SUPABASE_ENV" && ! grep -q "^DASHBOARD_PASSWORD=not_being_used" "$SUPABASE_ENV"; then
        DASHBOARD_PASSWORD=$(grep "^DASHBOARD_PASSWORD=" "$SUPABASE_ENV" | cut -d'=' -f2-)
    else
...
set_env_var "$SCRIPT_DIR/supabase/.env" "DASHBOARD_PASSWORD" "$DASHBOARD_PASSWORD"
...
if [ "$ACCESS_TYPE" = "1" ]; then
    ...
    echo "  • Studio Auth: supabase / ${DASHBOARD_PASSWORD}"
