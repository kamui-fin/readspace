...
# Note: Server always uses Docker internal addresses (kong:8000, db:5432)
# These are already set in docker-compose.yml, only creating minimal .env for local dev
cat <<EOF > "$PROJECT_ROOT/server/.env"
# This file was auto-generated by setup.sh
# Note: When running via docker-compose, most variables are set there

# Supabase configuration (for local development outside Docker)
