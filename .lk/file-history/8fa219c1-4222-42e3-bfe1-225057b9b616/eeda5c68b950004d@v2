...
down_revision: Union[str, None] = 'fdb333511dc0'
...
def upgrade() -> None:
    ...
    # Primary lookup index for user article states (critical for all article queries)
    # Covers: user_id + article_id lookups in article list queries
    op.create_index(
    ...
    # Composite index for duplicate checking during article insertion
    # Covers: (feed_id, guid) uniqueness checks
    op.create_index(
    ...
    op.create_index(
        'idx_feed_articles_feed_date',
        'feed_articles',
        ['feed_id', sa.text('created_at DESC')],
        unique=False
    )
    ...
    # Covers: search queries across title and description
    # Note: Using CONCURRENTLY to avoid blocking production tables
    op.execute("""
        CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_article_content_fts
    ...
    # Index for feed refresh prioritization
...
def downgrade() -> None:
    ...
    op.execute('DROP INDEX CONCURRENTLY IF EXISTS idx_article_content_fts')
