...
function discoverRSSFeeds() {
  ...
  const feedLinks = document.querySelectorAll('link[type="application/rss+xml"], link[type="application/atom+xml"], link[type="application/json"], link[rel="alternate"]')
...
async function extractArticleContent() {
  ...
  try {
    ...
    console.log('Content preview (first 500 chars):', result.content?.substring(0, 500))
    ...
    console.log('Content will be sent to backend - length:', extractedData.content?.length || 0)
...
function basicContentExtraction() {
  ...
  if (!content) {
    ...
    console.log(`Extracted ${paragraphs.length} paragraphs, total HTML length: ${content.length}`)
  }
