 ...
 */
function trimString(value: string | null | undefined, maxLength: number): string | undefined {
...
export function trimSaveArticleRequest(data: {
  ...
  const trimmed = {
    ...
    metadata: data.metadata ? {
      description: trimString(data.metadata.description, MAX_DESCRIPTION_LENGTH),
  ...
  if (trimmed.metadata) {
    ...
    if (trimmed.metadata.description !== undefined) cleanMetadata.description = trimmed.metadata.description
    if (trimmed.metadata.author !== undefined) cleanMetadata.author = trimmed.metadata.author
    if (trimmed.metadata.published_at !== undefined) cleanMetadata.published_at = trimmed.metadata.published_at
    if (trimmed.metadata.image_url !== undefined) cleanMetadata.image_url = trimmed.metadata.image_url
    if (trimmed.metadata.favicon !== undefined) cleanMetadata.favicon = trimmed.metadata.favicon
    
    trimmed.metadata = Object.keys(cleanMetadata).length > 0 ? (cleanMetadata as any) : undefined
  }

  // Remove undefined values to keep the request clean
  return Object.fromEntries(
    Object.entries(trimmed).filter(([_, value]) => value !== undefined)
  ) as any
}