...
export function useExtractFullText(articleId: string) {
    return useQuery({
        ...
        queryFn: async (): Promise<ExtractFullTextResponse> => {
            const response = await ApiClient.post<ExtractFullTextResponse>(`/api/articles/${articleId}/extract-full-text`)
            return response
        },
 ...
 */
export function createTranslationQueryKey(articleId: string, targetLanguage: string, content?: string) {
    const contentHash = content ? createContentHash(content) : 'original'
    return [ARTICLE_ENHANCEMENT_QUERY_KEYS.TRANSLATION, articleId, targetLanguage, contentHash]
}
