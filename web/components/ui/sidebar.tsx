...
import { PanelLeft, TableOfContents } from "lucide-react"
...
const SIDEBAR_RIGHT_WIDTH = "27rem"
...
const SidebarLeftContext = React.createContext<SidebarContextProps | null>(null)
            ...
            <SidebarContext.Provider value={contextValue}>
                <TooltipProvider delayDuration={0}>
                    <div
                        ...
                        style={
                            {
                                "--sidebar-left-width": SIDEBAR_LEFT_WIDTH,
    ...
    (
        ...
        if (collapsible === "none") {
            return (
                <div
                    className={cn(
                        "bg-sidebar text-sidebar-foreground flex h-full flex-col w-[var(--width-var)]",
                        className
                    )}
                    style={
                        {
        ...
        if (isMobile) {
            return (
                ...
                >
                    <SheetContent
                        className="bg-sidebar text-sidebar-foreground w-[var(--width-var)] p-0 [&>button]:hidden z-[1000]"
                        data-mobile="true"
                        data-sidebar="sidebar"
                        side={side}
                        style={
                            {
                                "--width-var": SIDEBAR_WIDTH_MOBILE,
                            } as React.CSSProperties
                        }
                    >
                        <SheetHeader>
...
function SidebarLeftMenuButton(props: SidebarMenuButtonProps) {
    const { isMobile, state, toggleSidebar } = useSidebarLeft()
...
function SidebarRightMenuButton(props: SidebarMenuButtonProps) {
    const { isMobile, state, toggleSidebar } = useSidebarRight()
