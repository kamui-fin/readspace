...
import { AlertTriangle, CheckCircle, Edit3, ExternalLink, MoreHorizontal, Trash2 } from "lucide-react";
...
import { useDeleteFeed, useFeeds, useFolders, useUpdateFeed, type Feed } from "@/lib/api/hooks/feeds";
import { fuzzySearch } from "@/lib/utils";
import { useDebounce } from "use-debounce";

// Placeholder for now, will be fleshed out
export default function ManageFeedsPage() {
    const { data: feeds = [], isLoading: isLoadingFeeds, error: feedsError } = useFeeds();
    ...
    const [isBulkEditFolderModalOpen, setIsBulkEditFolderModalOpen] = useState(false);
    const [targetBulkFolderId, setTargetBulkFolderId] = useState<string | null>(null);
    ...
    const filteredFeeds = useMemo(() => {
        ...
        if (filterFolderId !== "all") {
            tempFeeds = tempFeeds.filter(feed => feed.folder_id === filterFolderId);
        }
        ...
        if (debouncedSearchTerm) {
            ...
            // );
            tempFeeds = fuzzySearch(tempFeeds, debouncedSearchTerm, ['title', 'url']);
        }
        return tempFeeds;
    ...
    const getLastUpdateTime = (feed: Feed): string => {
        if (feed.last_fetched_at) {
            return formatDistanceToNow(parseISO(feed.last_fetched_at), { addSuffix: true });
        }
        return "Never";
    };
    ...
    const isFeedDead = (feed: Feed): boolean => {
        ...
        if (feed.last_article_published_at) {
            ...
            if (feed.last_fetched_at && feed.fetch_error_count === 0) { // Successfully fetched but no articles
    ...
    const handleChangeFolder = (feedId: string, newFolderId: string | null) => {
        ...
        updateFeedMutation.mutate(
            { feedId, data: { folder_id: newFolderId === 'none' ? undefined : newFolderId } },
            {
    ...
    const handleDeleteSelected = () => {
        ...
        if (window.confirm(`Are you sure you want to delete ${selectedFeedIds.length} feed(s)?`)) {
            selectedFeedIds.forEach(id => {
                deleteFeedMutation.mutate(id, {
                    // onSuccess: () => toast.success(`Feed deleted.`), // Individual toasts can be noisy
                    // onError: () => toast.error(`Failed to delete feed ${id}.`),
                });
            });
            // Add a summary toast after attempting all
            toast.promise(
                Promise.allSettled(selectedFeedIds.map(id => deleteFeedMutation.mutateAsync(id))),
                {
    ...
    if (isLoadingFeeds || isLoadingFolders) {
        return <div className="container mx-auto p-4">Loading manage feeds page...</div>;
    }

    if (feedsError) {
        return <div className="container mx-auto p-4 text-red-500">Error loading feeds: {(feedsError as Error).message}</div>;
    }
        ...
        <div className="container mx-auto p-4 md:p-8 space-y-6">
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground">View, edit, and organize your RSS feeds.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                ...
                <div className="md:col-span-1 flex justify-end gap-2">
                    {selectedFeedIds.length > 0 && (
                        <Button variant="outline" onClick={() => setIsBulkEditFolderModalOpen(true)}>
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead className="hidden md:table-cell text-right">Last Active</TableHead>
                    ...
                    <TableBody>
                        ...
                        {filteredFeeds.map(feed => {
                            ...
                            return (
                                <TableRow key={feed.id} data-state={isSelected ? "selected" : undefined}>
                                    <TableCell>
                                        <Checkbox
                                            checked={isSelected}
                                            onCheckedChange={(checked) => handleSelectFeed(feed.id, Boolean(checked))}
                                            aria-label={`Select row for ${feed.title}`}
                                        />
                                    </TableCell>
                                    <TableCell className="font-medium">
                                        <div className="flex items-center gap-2">
                                            {feed.image_url && <img src={feed.image_url} alt="" className="h-5 w-5 rounded-sm object-cover" />}
                                            <div className="flex flex-col">
                                                <span className="truncate max-w-[200px] md:max-w-[300px]" title={feed.title || "N/A"}>{feed.title || "N/A"}</span>
                                                <a
                                                    href={feed.url}
                                                    target="_blank"
                                                    ...
                                                    title={feed.url}
                                                >
                                                    {feed.url} <ExternalLink className="inline h-3 w-3 ml-0.5" />
                                                </a>
                                            </div>
                                        </div>
                                    </TableCell>
                                    <TableCell className="hidden md:table-cell">
                                        <Select
                                            ...
                                            onValueChange={(newFolderId) => handleChangeFolder(feed.id, newFolderId)}
                                        >
                                    ...
                                    <TableCell className="hidden lg:table-cell">
                                        {feed.tags.length > 0
                                            ? feed.tags.map(tag => <Badge key={tag.id} variant="outline" className="mr-1 mb-1 text-xs">{tag.name}</Badge>)
                                    ...
                                    <TableCell className="hidden lg:table-cell text-center">
                                        {dead ? (
                                            <Badge variant="destructive" title={`Error count: ${feed.fetch_error_count}. ${feed.last_error_message || ''}`}>
                                                <AlertTriangle className="h-3 w-3 mr-1" /> Dead
                                            </Badge>
                                        ) : feed.fetch_error_count > 0 ? (
                                            <Badge variant="orange" title={`Error count: ${feed.fetch_error_count}. ${feed.last_error_message || ''}`}>
                                    ...
                                    <TableCell className="hidden md:table-cell text-right text-xs">
                                        ...
                                        {feed.last_article_published_at && (
                                            <div className="text-muted-foreground/80">
                                                Last post: {formatDistanceToNow(parseISO(feed.last_article_published_at), { addSuffix: true })}
                                            </div>
                                        )}
                                        {feed.last_error_message && (
                                            <div className="text-destructive/80" title={feed.last_error_message}>
                                    ...
                                    <TableCell className="text-right">
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end">
                                                ...
                                                >
                                                    <Trash2 className="mr-2 h-4 w-4" /> Delete Feed
                                                </DropdownMenuItem>
                                            </DropdownMenuContent>
                                        </DropdownMenu>
                                    </TableCell>
            ...
            {currentFeed && (
                <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>Edit Feed: {currentFeed.title}</DialogTitle>
                            <DialogDescription>
                                Current URL: {currentFeed.url}
                            </DialogDescription>
                        </DialogHeader>
                        <div className="py-4">
                            <label htmlFor="feedTitle" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                Feed Title
                            </label>
                            <Input
                                id="feedTitle"
                        ...
                        <DialogFooter>
                            ...
                            <Button
                                onClick={() => {
                                    ...
                                    updateFeedMutation.mutate(
                                        { feedId: currentFeed.id, data: { title: editFeedTitle.trim() } },
                                        {
                                            onSuccess: () => {
                                                toast.success("Feed title updated.");
                                                setIsEditModalOpen(false);
                                            },
                                            onError: () => toast.error("Failed to update feed title."),
                                        }
                                    );
                                }}
                                disabled={updateFeedMutation.status === "pending" || editFeedTitle.trim() === (currentFeed.title || "")}
                            >
                                {updateFeedMutation.status === "pending" ? "Saving..." : "Save Changes"}
                            </Button>
                        </DialogFooter>
                    </DialogContent>
                </Dialog>
            ...
            {currentFeed && (
                <Dialog open={isTagsModalOpen} onOpenChange={setIsTagsModalOpen}>
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>Manage Tags for: {currentFeed.title}</DialogTitle>
                            <DialogDescription>
                                Current tags:
                                {currentFeed.tags.length > 0
                                    ? currentFeed.tags.map(tag => (
                                        <Badge key={tag.id} variant="secondary" className="mr-1 my-2 text-xs">{tag.name}</Badge>
                                    ))
                                    : <span className="ml-1 italic text-muted-foreground">No tags assigned.</span>
                                }
                            </DialogDescription>
                        </DialogHeader>
                        <div className="py-4">
                            {/* Placeholder for adding/removing tags */}
                            <p className="text-sm text-muted-foreground">Tag editing functionality (add, remove new/existing tags) will be implemented here.</p>
                            <Input disabled placeholder="Add new or existing tag... (future feature)" className="mt-2" />
                        </div>
                        <DialogFooter>
                            <Button variant="outline" onClick={() => setIsTagsModalOpen(false)}>Close</Button>
            ...
            {currentFeed && (
                <Dialog open={isDeleteConfirmModalOpen} onOpenChange={setIsDeleteConfirmModalOpen}>
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>Delete Feed: {currentFeed.title}?</DialogTitle>
                            <DialogDescription>
                                Are you sure you want to unfollow and delete this feed? This action cannot be undone.
                                All downloaded articles for this feed will also be removed.
                            </DialogDescription>
                        </DialogHeader>
                        <DialogFooter className="mt-4">
                            <Button variant="outline" onClick={() => setIsDeleteConfirmModalOpen(false)} disabled={deleteFeedMutation.status === "pending"}>
                                Cancel
                            </Button>
                            <Button
                                variant="destructive"
                                onClick={() => {
                                    deleteFeedMutation.mutate(currentFeed.id, {
                                        onSuccess: () => {
                                            toast.success(`Feed "${currentFeed.title}" deleted.`);
                                            setIsDeleteConfirmModalOpen(false);
                                            if (selectedFeedIds.includes(currentFeed.id)) {
                                                setSelectedFeedIds(prev => prev.filter(id => id !== currentFeed.id));
                                            }
                                            setCurrentFeed(null); // Clear current feed after deletion
                                        },
                                        onError: (err: any) => {
                                            toast.error(err.message || `Failed to delete feed "${currentFeed.title}".`);
                                            setIsDeleteConfirmModalOpen(false);
                                        },
                                    });
                                }}
                            ...
                            >
                                {deleteFeedMutation.status === "pending" ? "Deleting..." : "Delete Feed"}
                            </Button>
                        </DialogFooter>
                    </DialogContent>
                </Dialog>
            ...
            <Dialog open={isBulkEditFolderModalOpen} onOpenChange={(isOpen) => {
                setIsBulkEditFolderModalOpen(isOpen);
                if (!isOpen) setTargetBulkFolderId(null); // Reset on close
            }}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Change Folder for {selectedFeedIds.length} Feed(s)</DialogTitle>
                        <DialogDescription>
                            Select a new folder for the selected feeds. This will update all selected feeds.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="py-4">
                        <Select
                        ...
                        >
                            ...
                            <SelectContent>
                                <SelectItem value="none">No Folder (Remove from current folder)</SelectItem>
                                {folders.map(folder => (
                                    <SelectItem key={folder.id} value={folder.id}>{folder.name}</SelectItem>
                                ))}
                            </SelectContent>
                    ...
                    <DialogFooter>
                        ...
                        <Button
                            onClick={() => {
                                ...
                                const updates = selectedFeedIds.map(feedId =>
                                    updateFeedMutation.mutateAsync({
                                        feedId,
                                        data: { folder_id: targetBulkFolderId === 'none' ? undefined : targetBulkFolderId }
                                    })
                                );
                            ...
                            disabled={!targetBulkFolderId || updateFeedMutation.status === "pending"}
                        >
                            Move Feeds
                        </Button>
                    </DialogFooter>
...
// const fuzzySearch = (items: Feed[], term: string, keys: (keyof Feed)[]): Feed[] => {
...
// Create a useDebounce hook if it doesn't exist (e.g., in @/hooks/use-debounce.ts)
